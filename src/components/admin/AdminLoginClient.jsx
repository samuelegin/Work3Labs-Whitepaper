  ...
  return (
    <div>
      ...
      <input id={id} type={type} value={value} onChange={e => onChange(e.target.value)}
        ...
        autoFocus={autoFocus} aria-invalid={Boolean(error)}
        aria-describedby={error ? `${id}-error` : undefined} className={inputCls(Boolean(error))} />
      {error && (
...
export default function AdminLoginClient() {
  ...
  function handleReset() { setEmail(''); setPassword(''); setErrors({}); emailRef.current?.focus() }
  ...
  async function handleSubmit() {
    ...
    if (error) {
      const msg = error.includes('401') || error.toLowerCase().includes('invalid') || error.toLowerCase().includes('credential')
        ? 'Invalid email or password. Please try again.'
        : error.includes('429') ? 'Too many attempts. Please wait a moment and try again.'
        ...
        <div className="w-full max-w-[400px]" style={{ animation: 'up 0.5s cubic-bezier(0.22,1,0.36,1) both' }}>
          ...
          <div className="bg-white border border-black/[0.07] rounded-2xl shadow-[0_2px_40px_rgba(0,0,0,0.06)] overflow-hidden">
            <div className="px-7 pt-7 pb-6 border-b border-black/[0.06]">
              ...
              <p className="text-[13px] font-light text-[#AAA]">Work3 Labs admin dashboard</p>
            </div>
            <div className="px-7 py-6 space-y-4">
              ...
              <Field id="email" label="Email address" type="email" value={email}
                ...
                onKeyDown={handleKeyDown} placeholder="admin@work3labs.com" autoComplete="email"
              ...
              <div className="flex justify-end">
                <Link href="/admin/forgot-password" className="font-mono text-[10px] tracking-[0.08em] uppercase text-[#BBB] hover:text-ink transition-colors">
                  Forgot password?
                </Link>
              </div>
            </div>
            <div className="px-7 pb-7 space-y-2.5">
              <button onClick={handleSubmit} disabled={submitting}
                ...
                {submitting ? <><Spinner /><span>Signing in…</span></> : <><i className="bi bi-box-arrow-in-right text-[15px]" />Sign in</>}
              </button>
          ...
          <p className="text-center font-mono text-[10px] tracking-[0.08em] text-[#CCC] mt-6">Work3 Labs · Admin Access Only</p>
        </div>
      </div>
    </div>
  )
